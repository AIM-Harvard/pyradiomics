...
# indivdual segments will be extracted to the destination directory into NRRD
...
def writeSR(inputSEG, inputJSON, inputDICOMImageDir, outputSR):
  cmd = ['tid1500writer', '--inputImageLibraryDirectory', inputDICOMImageDir, '--inputCompositeContextDirectory',
         os.path.split(inputSEG)[0], '--inputMetadata', inputJSON, '--outputDICOM', outputSR]
  ...
  def addMeasurement(self, value, quantityCode, unitsCode=CodedValue("1", "UCUM", "no units")):
    ...
    if featureTuple.empty:
      ...
      measurement["quantity"] = \
        CodedValue(featureTuple["IBSI_code"].values[0], "IBSI", featureTuple["IBSI_meaning"].values[0]).getDict()
    ...
    try:
      if numpy.isnan(value):
        ...
        print("Exception with nan: "+str(e)+" "+value)
        return
...
def main():
  ...
  parser.add_argument('--input-image-dir', dest="inputDICOMImageDir", metavar='Input DICOM image directory',
  ...
  parser.add_argument('--input-seg-file', dest="inputSEG", metavar='Input DICOM SEG file', help="Input segmentation defined as a"
  ...
  parser.add_argument('--volume-reconstructor', dest="volumeReconstructor",
                      metavar="Choose the tool to be used for reconstructing image volume from the DICOM image series. Allowed options are plastimatch or dcm2niix (should be installed on the system). plastimatch will be used by default.", default="plastimatch")
  ...
  if args.volumeReconstructor == "plastimatch":
    ...
    scriptlogger.error("Unknown option for volume reconstruction: %s. Allowed values are plastimatch and dcm2niix.")
    return -1
  ...
  for inputSegment in inputSegments:
    ...
    try:
      ...
      # the command line.
      correctMaskSetting = {}
      correctMaskSetting["setting"] = {"geometryTolerance": 0.001, "correctMask": True}
      extractor = featureextractor.RadiomicsFeaturesExtractor(parameters)
      if args.parameters is not None:
        extractor.loadParams(paramsFile=args.parameters)
      extractor.settings.update(correctMaskSetting)

    except Exception as e:
      scriptlogger.error('Initialization of the pyradimics feature extraction failed.:'+str(e))
      return -1
    ...
    scriptlogger.debug("%d of %d total features included in the TID 1500 Measurement group."
...
if __name__ == "__main__":
  for exe in ['tid1500writer', 'dcm2niix', 'segimage2itkimage']:
    if distutils.spawn.find_executable(exe) is None:
      ...
      print("or Plastimatch (http://plastimatch.org/)")
