...
class RadiomicsGLCM(base.RadiomicsFeaturesBase):
  ...
  :math:`P(i,j)` be the co-occurence matrix for an arbitrary :math:`\delta` and :math:`\alpha`
  ...
  :math:`HY =  -\sum^{N_g}_{j=1}{p_y(j)\log_2\big(p_y(j)\big)}` be the entropy of :math:`p_y`

  :math:`HXY =  -\sum^{N_g}_{i=1}\sum^{N_g}_{j=1}{\textbf{P}(i,j)\log_2\big(\textbf{P}(i,j)\big)}` be the entropy of :math:`P(i,j)`
