...
class RadiomicsHelpers:
  ...
  def binImage(binwidth, parameterValues, parameterMatrix, parameterMatrixCoordinates):
    """
    Bin the voxel intensity levels within the image. 
    
    Bin the voxel intensity levels within the image based on binwidth and center bin edges
    at zero. 
    """
  ...
  def imageToNumpyArray(imageFilePath):
    """Read image into a SimpleITK object and convert into a numpy array."""
  ...
  def padCubicMatrix(a, matrixCoordinates, padDistance):
    ...
    """
    dims = tuple(map(operator.add, a.shape, tuple(np.tile(padDistance,3))))
