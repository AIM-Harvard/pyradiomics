...
class DatasetHierarchyReader:
    ...
    def ReadDatasetHierarchy(self, create=False):
        ...
        for patientDirectory in patientDirectories:
            ...
            for studyDirectory in studyDirectories:
                self.DatabaseHierarchyDict[patientDirectory][
                    studyDirectory
                ] = collections.OrderedDict()

                subfolders = [
                    dirpath
                    for dirpath in glob.glob(os.path.join(studyDirectory, "*"))
                    if os.path.isdir(dirpath)
                ]

                reconstructionsDirectory, images = self.readReconstructionsDirectory(
                    studyDirectory, subfolders, create=create
                )
                self.DatabaseHierarchyDict[patientDirectory][studyDirectory][
                    "reconstructions"
                ] = images

                resourcesDirectory, resources = self.readResourcesDirectory(
                    studyDirectory, subfolders, create=create
                )
                self.DatabaseHierarchyDict[patientDirectory][studyDirectory][
                    "resources"
                ...
                segmentationsDirectory, labels = self.readSegmentationsDirectory(
                    studyDirectory, subfolders, create=create
                )
                self.DatabaseHierarchyDict[patientDirectory][studyDirectory][
                    "segmentations"
    ...
    def readReconstructionsDirectory(self, studyDirectory, subfolders, create=False):
        ...
        try:
            ...
            images = [
                ...
                if self.filetype in os.path.basename(item)
            ]
        except IndexError:
            if create:
    ...
    def findImageAndLabelPair(self, imageFilepaths, maskFilepaths, keywordSettings):
        ...
        keywordSettings = {
            k: [str(keyword.strip()) for keyword in v.split(",")]
        ...
        for maskFilepath in maskFilepaths:
            ...
            if self.testString(
                maskFilename, keywordSettings["mask"], keywordSettings["maskExclusion"]
            ):
    ...
    def testString(self, fileName, inclusionKeywords, exclusionKeywords):
        ...
        inclusionKeywords = [
            keyword.upper() for keyword in inclusionKeywords if (keyword != "")
        ]
        exclusionKeywords = [
            keyword.upper() for keyword in exclusionKeywords if (keyword != "")
        ]
